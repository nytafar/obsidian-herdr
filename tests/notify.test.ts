...
function harness(settingsOverride?: Partial<NotificationSettings>): Harness {
	...
	const deps: NotifierDeps = {
		now: () => clock.now,
		settings: () => settings,
		isTerminalOpen: (paneId) => terminalOpen.has(paneId),
...
describe('decideNotification', () => {
	it('escalates to an OS notification only while the window is unfocused', () => {
		...
		const focusedPlan = decideNotification(pane('working'), pane('blocked'), undefined, h.deps);
		expect(focusedPlan?.notice).toBe(true);
		expect(focusedPlan?.os).toBe(false);

		h.focused.value = false;
		const blurredPlan = decideNotification(pane('working'), pane('blocked'), undefined, h.deps);
	...
	it('respects the per-transition settings', () => {
		...
		expect(decideNotification(pane('working'), pane('blocked'), undefined, h.deps)).toBeNull();
		const donePlan = decideNotification(pane('working'), pane('done'), undefined, h.deps);
		expect(donePlan).toMatchObject({ notice: false, os: true, transition: 'done' });
	});

	it('skips panes whose terminal view is open', () => {
		const h = harness();
		h.terminalOpen.add('w4:p1');
		expect(decideNotification(pane('working'), pane('blocked'), undefined, h.deps)).toBeNull();
	});
	...
	it('skips a pane that fired less than the debounce window ago', () => {
		const h = harness();
		h.clock.now = 5000;
		expect(decideNotification(pane('working'), pane('blocked'), 5000 - DEBOUNCE_MS + 1, h.deps)).toBeNull();
		expect(decideNotification(pane('working'), pane('blocked'), 5000 - DEBOUNCE_MS, h.deps)).not.toBeNull();
	});
});
...
describe('TransitionNotifier', () => {
	...
	it('notifies once and then mutes the pane for the debounce window', () => {
		h.notifier.onChanged(pane('working'), pane('blocked'));
		expect(h.notices).toHaveLength(1);
		expect(h.os).toHaveLength(1);

		// Flapping blocked -> working -> blocked inside 2 s stays quiet.
		h.clock.now += 500;
		h.notifier.onChanged(pane('blocked'), pane('working'));
		h.clock.now += 500;
		h.notifier.onChanged(pane('working'), pane('blocked'));
		expect(h.notices).toHaveLength(1);
	...
	it('debounces per pane, not globally', () => {
		...
		h.notifier.onChanged(pane('working', other), pane('blocked', other));
		expect(h.notices).toHaveLength(2);
	});

	it('forgets a pane that left the scope', () => {
		h.notifier.onChanged(pane('working'), pane('done'));
		h.notifier.forget('w4:p1');
		h.clock.now += 10;
		const plan: NotifyPlan | null = h.notifier.onChanged(pane('working'), pane('done'));
